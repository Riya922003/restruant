    ...
    <Card className="flex flex-col p-5">
      ...
      <div className="mt-4 flex-1">
        {loading ? (
              ...
              <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-zinc-100 pt-3">
                ...
                {Object.entries(data.context).map(([k, v]) => (
                  <span key={k} className="rounded-full bg-zinc-100 px-2 py-0.5 text-[11px] text-zinc-600">
                    {humanize(k)}: <span className="font-medium text-zinc-800">{String(v)}</span>
                  </span>
                ))}
              </div>
            ) : null}
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50/60 px-4 py-8 text-center">
            <span className="mb-2 text-2xl opacity-60">{icon}</span>
            <p className="text-sm text-zinc-500">
              Hit <span className="font-medium text-zinc-700">{buttonLabel}</span> for an AI suggestion from your live data.
            </p>
          </div>
        )}
      </div>
...
export const TD = "px-3 py-2.5 text-sm text-zinc-700 align-middle";
